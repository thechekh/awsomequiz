...
/* pages/login.py uses st.link_button (NOT raw HTML) so the link escapes
   ...
   a ::before pseudo-element on the <a>. As a pseudo-element it becomes
   a flex sibling of the label, so both center together as a unit instead
   of the icon being anchored to the button's left edge. */
.stLinkButton a[href*="provider=github"]::before {
    content: "";
