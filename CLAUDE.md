...
- **`get_oauth_supabase()`** -- a process-shared `@st.cache_resource` client used ONLY for the PKCE auth handshake (`get_github_oauth_url` / `exchange_code` / `reset_password_request`). The PKCE code-verifier is created when the auth URL is built and must still exist at the callback, which arrives in a *different* session -- a per-session client would have lost it (this regressed GitHub login until the shared OAuth client was added). `exchange_code` resets it to anon after a successful exchange so it never retains a token.
